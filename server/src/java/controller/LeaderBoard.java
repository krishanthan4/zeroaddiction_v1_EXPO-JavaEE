...
@Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            // Query to retrieve all usage tracking data, ordered by username and date descending
            String hql = "SELECT u.user.username, u.date, u.count " +
                         "FROM UsageTracking u INNER JOIN u.user user " +
                         "WHERE u.date <= :currentDate " +
                         "ORDER BY u.user.username, u.date DESC";
                ...
                for (Object[] row : resultList) {
                    ...
                    if (currentUser == null || !currentUser.equals(username)) {
                        if (currentUser != null && streakDays >= 2) {
                            ...
                            singleObject.put("username", currentUser);
